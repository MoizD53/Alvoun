    ...
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Real-time GPS tracking of field salesmen.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm flex items-center gap-2 text-sm font-medium">
            <span className="text-slate-500">Active Today:</span>
            <span className="text-slate-900">{activeCount}</span>
          </div>
          <div className="bg-green-50 px-4 py-2 rounded-lg border border-green-100 flex items-center gap-2 text-sm font-medium">
              ...
              {displayData.map((s) => (
                <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                    ...
                    <div className="text-xs text-slate-500">{s.employeeCode}</div>
                  </td>
                  <td className="px-6 py-4">
                    {s.isWorking ? (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-green-50 text-green-700 border border-green-100">
                        <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span> Working
                      </span>
                    ) : (
                  ...
                  <td className="px-6 py-4 text-center">
                    ...
                    {s.status === 'Offline' && <span className="px-2.5 py-1 bg-slate-100 text-slate-500 rounded-lg text-xs font-medium border border-slate-200">Offline</span>}
                  </td>
                  <td className="px-6 py-4 text-right font-medium text-slate-700">{s.lastUpdated}</td>
                  <td className="px-6 py-4 text-right text-slate-500 text-sm">{s.ageText}</td>
                </tr>
              ))}
            </tbody>
          </table>
