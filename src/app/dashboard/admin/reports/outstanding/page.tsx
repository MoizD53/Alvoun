  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Outstanding Report</h1>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <form className="flex flex-wrap items-center gap-2 bg-white p-1 rounded-md border border-slate-200 shadow-sm">
            ...
            <div className="w-px h-5 bg-slate-200"></div>
            <label className="flex items-center gap-2 text-sm font-medium text-slate-700 px-2 cursor-pointer">
              <input type="checkbox" name="outstandingOnly" value="true" defaultChecked={outstandingOnly} className="rounded border-slate-300 text-alvoun-blue focus:ring-alvoun-blue" />
              Owing Only
            </label>
                ...
                reportData.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-3">
                      ...
                      <div className="text-xs text-slate-500">{c.city.name}, {c.state.name}</div>
                    </td>
                    <td className="px-6 py-3">
                      <div className="font-medium text-slate-700">{c.route.name}</div>
                      <div className="text-xs text-slate-500">{c.salesman.name}</div>
                    </td>
                    <td className="px-6 py-3 text-right font-medium text-slate-700">{formatMoney(c.totalSales)}</td>
