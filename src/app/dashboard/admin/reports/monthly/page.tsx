  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <h2 className="text-xl font-bold text-slate-900">Monthly Sales Report</h2>
        <div className="flex items-center gap-3">
          <form className="flex items-center gap-2">
            <input type="number" name="year" defaultValue={year} className="w-24 px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50" />
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-50">
            {reportData.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-slate-500">No data for this year.</td>
              </tr>
            ) : (
              reportData.map((row) => {
                ...
                return (
                  <tr key={row.monthStr} className="hover:bg-slate-50">
                    <td className="px-4 py-3 font-medium text-slate-900">{monthName} {y}</td>
