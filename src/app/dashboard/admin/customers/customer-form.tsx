    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input required type="text" name="customerName" defaultValue={initialData?.customerName} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Contact Number *</label>
          <input required type="text" name="contact" defaultValue={initialData?.contact} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
        </div>
        <div className="md:col-span-2">
          <label className="block text-sm font-medium text-slate-700 mb-1">Address *</label>
          <textarea required name="address" defaultValue={initialData?.address} rows={3} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"></textarea>
        </div>
        ...
        <div>
          ...
          <select 
            ...
            disabled={!stateId}
            className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm bg-white disabled:bg-slate-100"
          >
        ...
        <div>
          ...
          <select required name="salesmanId" defaultValue={initialData?.salesmanId || ''} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm bg-white">
        ...
        <div>
          ...
          <input required type="number" step="0.01" min="0" name="openingBalance" defaultValue={initialData ? (initialData.openingBalance / 100).toFixed(2) : '0.00'} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Balance Type</label>
          <select name="openingBalanceType" defaultValue={initialData?.openingBalanceType || 'DEBIT'} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm bg-white">
        ...
        <div className="md:col-span-2">
          ...
          <select name="status" defaultValue={initialData?.status || 'ACTIVE'} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm bg-white">
      ...
      <div className="flex gap-4 pt-4 border-t border-slate-100">
        <button type="button" onClick={() => router.back()} className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-md hover:bg-slate-50">
          Cancel
        </button>
