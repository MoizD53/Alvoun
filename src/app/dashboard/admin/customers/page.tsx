    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage your customer database and assignments.</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <Link 
            href="/dashboard/admin/customers/import" 
            className="flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-slate-50 transition-colors flex-1 sm:flex-none shadow-sm"
          >
        ...
        <form className="flex flex-col lg:flex-row gap-3 items-center">
          <div className="relative w-full lg:w-96 flex-shrink-0">
            ...
            <input 
              ...
              className="w-full pl-10 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-alvoun-blue/20 focus:border-alvoun-blue transition-colors"
            />
          </div>
          
          <div className="flex w-full lg:w-auto gap-2 overflow-x-auto pb-1 lg:pb-0">
            <select name="routeId" defaultValue={filters.routeId} className="bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-alvoun-blue/20 focus:border-alvoun-blue min-w-[140px]">
              <option value="">All Routes</option>
              {routes.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
            </select>
            <select name="salesmanId" defaultValue={filters.salesmanId} className="bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-alvoun-blue/20 focus:border-alvoun-blue min-w-[140px]">
              <option value="">All Salesmen</option>
              {salesmen.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
            <select name="status" defaultValue={filters.status} className="bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-alvoun-blue/20 focus:border-alvoun-blue min-w-[120px]">
              <option value="">All Statuses</option>
              <option value="ACTIVE">Active</option>
              <option value="INACTIVE">Inactive</option>
            </select>
            <button type="submit" className="px-4 py-2 bg-slate-100 text-slate-700 rounded-md text-sm font-medium hover:bg-slate-200 transition-colors whitespace-nowrap border border-slate-200">
              Apply Filters
            </button>
          </div>
        </form>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center text-slate-400">
                      ...
                      <p className="text-sm mt-1 text-slate-500">Try adjusting your filters or add a new customer.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-slate-50 transition-colors group">
                      <td className="px-6 py-3">
                        ...
                        <div className="text-xs text-slate-500 mt-0.5">{c.contact} • {c.city.name}</div>
                      </td>
                      <td className="px-6 py-3">
                        <div className="font-medium text-slate-700">{c.route.name}</div>
                        <div className="text-xs text-slate-500 mt-0.5">{c.salesman.name}</div>
                      </td>
                      <td className="px-6 py-3 text-right">
                        <span className={`font-bold ${isNegative ? 'text-alvoun-red' : 'text-slate-900'}`}>
                      ...
                      <td className="px-6 py-3 text-right text-slate-500 font-medium text-sm">
                        {lastVisit}
                      </td>
                      <td className="px-6 py-3 text-center">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${c.status === 'ACTIVE' ? 'bg-green-50 text-alvoun-green' : 'bg-slate-100 text-slate-500'}`}>
                          {c.status}
                        </span>
                      </td>
                      <td className="px-6 py-3 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <a href={`tel:${c.contact}`} className="p-1.5 text-slate-400 hover:text-alvoun-blue bg-white hover:bg-alvoun-light border border-slate-200 rounded transition-colors" title="Call">
                            <Phone className="h-4 w-4" />
                          </a>
                          <Link href={`/dashboard/admin/customers/${c.id}`} className="flex items-center gap-1 px-2.5 py-1.5 bg-white border border-slate-200 text-slate-600 hover:text-alvoun-blue hover:border-alvoun-blue rounded text-xs font-medium transition-all shadow-sm">
                ...
                <div key={c.id} className="p-4 bg-white hover:bg-slate-50 transition-colors">
                  ...
                  <div className="grid grid-cols-2 gap-y-2 text-sm mt-3 pt-3 border-t border-slate-50">
                    <div>
                      ...
                      <div className="font-medium text-slate-700">{c.route.name}</div>
                      <div className="text-xs text-slate-500">{c.salesman.name}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-xs text-slate-400 font-medium">Outstanding</div>
                      <div className={`font-bold ${isNegative ? 'text-alvoun-red' : 'text-slate-900'}`}>
                        {formatMoney(Math.abs(outstanding))}
                      </div>
                    </div>
                  </div>
                  <div className="mt-4 flex gap-2">
                    <a href={`tel:${c.contact}`} className="flex-1 flex justify-center items-center gap-2 py-2 border border-slate-200 rounded-md text-sm font-medium text-slate-600 hover:bg-slate-50">
