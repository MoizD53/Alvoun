...
export default async function SalesmanCustomersPage({
  ...
  return (
    <div className="space-y-6 pb-20 animate-fade-in-up">
      <div className="sticky top-0 z-20 bg-slate-50 pt-2 pb-4 -mx-4 px-4 sm:mx-0 sm:px-0">
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight mb-4">My Customers</h1>
        ...
        <form className="relative">
          <input 
            ...
            className="w-full pl-11 pr-4 py-3.5 bg-white border border-slate-200 rounded-xl shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-alvoun-blue/50 focus:border-alvoun-blue transition-all placeholder-slate-400 font-medium"
          />
      ...
      {routes.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h2 className="text-lg font-bold text-slate-900 mb-1">No customers found</h2>
          ...
          {routes.map((route) => (
            <div key={route.routeName} className="space-y-3">
              <div className="flex items-center gap-2 px-1">
                ...
                <h2 className="font-bold text-slate-900 text-sm uppercase tracking-wider">{route.routeName}</h2>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 divide-y divide-slate-100 overflow-hidden">
                {route.customers.map((c: any) => (
                  <Link 
                    ...
                    className="flex items-center justify-between p-4 active:bg-slate-50 transition-colors group"
                  >
                    ...
                    <div className="flex items-center justify-center w-8 h-8 rounded-full bg-slate-50 text-slate-400 group-hover:bg-alvoun-light group-hover:text-alvoun-blue transition-colors shrink-0">
